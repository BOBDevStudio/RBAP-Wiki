...
* A new music system made by {% include BOB %} has been added. (Btw it uses a forked version of "[Zone+](https://devforum.roblox.com/t//397465)" to determine what zone you're in.)
...
* * * Entry: `print(HowMuchOfGameIsMadeBy{% include BOB %})`
Output: <s class="spoiler"><code>A lot of it as there are only a few things in this game that are not made by {% include BOB %}`</s>
* * * Entry: `local Answer = QuestionAnswerer("Why does {% include BOB %} make me type out fake lines of code into this fake console?") print(Answer)`
...
* * * Entry: `local Answer = QuestionAnswerer("Who is {% include BOB %}?") print(Answer)`
Output: <s class="spoiler"><code>{% include BOB %} is the creator (and owner) of this game and he has also created most stuff in this game`</s>
* * * Entry: `local Answer = QuestionAnswerer("Did {% include BOB %} write this question?") print(Answer)`
...
* Some guy (that totally isn't me ({% include BOB %})) with a stencil did some graffiti all around the map depicting various different fun facts... <s class="spoiler">some of them hidden...</s>
