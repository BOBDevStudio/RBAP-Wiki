...
* A new music system made by {% include BOB.html %} has been added. (Btw it uses a forked version of "[Zone+](https://devforum.roblox.com/t//397465)" to determine what zone you're in.)
...
* * * Entry: `print(HowMuchOfGameIsMadeBy{% include BOB.html %})`
Output: <s class="spoiler"><code>A lot of it as there are only a few things in this game that are not made by {% include BOB.html %}`</s>
* * * Entry: `local Answer = QuestionAnswerer("Why does {% include BOB.html %} make me type out fake lines of code into this fake console?") print(Answer)`
Output: <s class="spoiler"><code>Not even he remembers why`</s>
* * * Entry: `if true == false then print("What is logic?") end`
Output: <s class="spoiler"><code>What is logic?`</s>
* * * Entry: `print("Yes")`
Output: <s class="spoiler"><code>I already do enough work for you. You make me run the same lines of code over and over again constantly!`</s>
* * * Entry: `local Answer = QuestionAnswerer("Who is {% include BOB.html %}?") print(Answer)`
Output: <s class="spoiler"><code>{% include BOB.html %} is the creator (and owner) of this game and he has also created most stuff in this game`</s>
* * * Entry: `local Answer = QuestionAnswerer("Did {% include BOB.html %} write this question?") print(Answer)`
Output: <s class="spoiler"><code>:b`</s>
...
* Some guy (that totally isn't me ({% include BOB.html %})) with a stencil did some graffiti all around the map depicting various different fun facts... <s class="spoiler">some of them hidden...</s>
